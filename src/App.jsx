                ...
                {currentUser.role === 'ADMIN' ? (
                  <>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 mt-8 justify-center">
                      <button onClick={() => loadMock('historical')} id="btn-mock-historical"
                        className="group flex items-center justify-center gap-2 px-6 py-3 bg-brand-700/40 hover:bg-brand-600/50 text-brand-200 rounded-xl text-sm font-medium transition-all duration-200 border border-brand-600/30 hover:border-brand-500/50">
                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" />
                        </svg>
                        <span>Load Mock Historical Data</span>
                        <span className="text-xs text-brand-400 ml-1">(6 months)</span>
                      </button>
                      <button onClick={() => loadMock('single')} id="btn-mock-single"
                        className="flex items-center justify-center gap-2 px-6 py-3 bg-surface-800/80 hover:bg-surface-700/80 text-surface-200 hover:text-white rounded-xl text-sm font-medium transition-all duration-200 border border-surface-700 hover:border-surface-600">
                        Load Mock Single Month
                      </button>
                    </div>
                  </>
                ) : (
